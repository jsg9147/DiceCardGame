...
public class FirebaseManager : MonoBehaviour
{
    ...
    private IEnumerator Login(string _email, string _password)
    {
        ...
        //추후에 삭제하셈
#if UNITY_EDITOR
        var LoginTask = auth.SignInWithEmailAndPasswordAsync("test@test.com", "123456");
#else
    ...
    // 덱 데이터를 받아오는 함수
    // 로그인시 그냥 다 받아오게 할 예정
    private IEnumerator LoadDeckListData()
    {
        var DBTask = DBreference.Child("user").OrderByChild("deck").GetValueAsync();
        yield return new WaitUntil(predicate: () => DBTask.IsCompleted);
    }
}